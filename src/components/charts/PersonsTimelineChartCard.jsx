...
export default function PersonsTimelineChartCard({ people, t }) {
  ...
  const fmtShortMonth = useMemo(() => new Intl.DateTimeFormat(locale, { month: 'short' }), [locale])
  ...
  const { data, title, avg, maxValue, canNext } = useMemo(() => {
    ...
    if (gran === 'month') {
      ...
      const lblSet = new Set([0, Math.round(dim / 3) - 1, Math.round((2 * dim) / 3) - 1, dim - 1])
      ...
      const items = Array.from({ length: dim }, (_, i) => {
        ...
        return {
          label: lbl,
          tooltipLabel: `${String(day).padStart(2, '0')}-${mm}`,
    ...
    if (gran === 'historic') {
      ...
      if (rangeDays < 7) {
        ...
        return {
          ...
          maxValue: Math.max(...items.map((x) => x.value), 1),
          canNext: false,
        }
      }
      ...
      if (rangeDays <= 365) {
        const items = Array.from({ length: rangeDays + 1 }, (_, i) => {
          ...
          d.setDate(firstDate.getDate() + i)
          const label =
            d.getDate() === 1 ? capitalize(fmtShortMonth.format(d).replace(/\.$/, '')) : ''
      ...
      const yr1 = firstDate.getFullYear(),
        mo1 = firstDate.getMonth()
      const yr2 = lastDate.getFullYear(),
        mo2 = lastDate.getMonth()
      const items = []
      let y = yr1,
        m = mo1
      ...
      const janIndices = items.reduce((acc, item, i) => {
        if (item.label) acc.push(i)
        return acc
      }, [])
    ...
    <FullscreenChartWrapper>
      ...
      >
        <CardHeader style={{ paddingBottom: '0.25rem' }}>
          <CardTitle style={{ ...TEXT.title, color: P.text }}>{t.personsTimelineTitle}</CardTitle>
          ...
          <p style={{ ...TEXT.body, color: P.textSoft, textAlign: 'center', marginTop: '0.25rem' }}>
            {avg % 1 === 0 ? avg : avg.toFixed(1)} {avg === 1 ? t.chartPerson : t.chartPersons}{' '}
            {t.timelineAvg}
          </p>
        </CardHeader>
        ...
        <CardContent>
          <div
            ...
            style={{ height: '15rem', width: '100%', position: 'relative' }}
          >
            <span
              style={{
                position: 'absolute',
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                <CartesianGrid vertical={false} strokeDasharray="3 3" stroke={P.cardBorder} />
                <XAxis
                  dataKey="xKey"
                ...
                <Tooltip
                  ...
                  content={
                    <TooltipContent
                      ...
                      noPersonsLabel={t.noPersons}
                      onLabel={t.timelineOn}
                    />
                  }
                />
