...
export default function ActivityDonutCard({ personsByActivity, eventsByActivity, emptyText, t }) {
  ...
  const title = mode === 'persons' ? t.personsByActivity : t.eventsByActivity

  const subtitle = mode === 'persons' ? t.activityDistribution : t.groupedByActivity
  ...
  const fills = data.map((_, index) => chartColors[index % chartColors.length])
